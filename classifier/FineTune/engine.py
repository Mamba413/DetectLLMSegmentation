...
def evaluate_model2(model, data, device, verbose=True, track_compute=False):
    model.to(device)
    model.eval()
    eval_loader = DataLoader(data, batch_size=1, shuffle=False)
    epoch_crit_train_original, epoch_crit_train_sampled = [],[]
    var_crit_train_original, var_crit_train_sampled = [], []
    time_list = []
    memory_list = []
    tracker = GpuMem()
    with torch.no_grad(), torch.inference_mode():
        for i, batch in enumerate(tqdm.tqdm(eval_loader, desc="Evaluating")):
            ...
            if track_compute:
                start = time.perf_counter()
                with tracker:
                    output = model.predict(text, training_module=False)
                time_list.append(time.perf_counter() - start)
                memory_list.append(tracker.memory_usage())
            else:
                output = model.predict(text, training_module=False)
            ...
            epoch_crit_train_original.extend(output['crit'][0].tolist())
            epoch_crit_train_sampled.extend(output['crit'][2].tolist())
            var_crit_train_original.extend(output['crit'][1].tolist())
            var_crit_train_sampled.extend(output['crit'][3].tolist())
    ...
    results_dict = {
        "name": "AdaJASAdetectgpt",
        'info': {'n_samples': len(epoch_crit_train_original)},
        'bias': {'real': epoch_crit_train_original, 'samples': epoch_crit_train_sampled},
        'var': {'real': var_crit_train_original, 'samples': var_crit_train_sampled},
        'metrics': {'roc_auc': roc_auc, 'fpr': fpr, 'tpr': tpr},
        'pr_metrics': {'pr_auc': pr_auc, 'precision': p, 'recall': r},
        'runtime': time_list, 
        'memory': memory_list,
