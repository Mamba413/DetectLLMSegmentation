...
def get_sampling_discrepancy(logits_ref, logits_score, labels):
    ...
    discrepancy = torch.nan_to_num(discrepancy, nan=-1e4)
...
def experiment(args):
    ...
    # load prompt data
    if args.prompt_file is not None and args.prompt_file != "":
        with open(args.prompt_file, 'r') as fin:
            sampled_prompt_data = json.load(fin)
    else:
        sampled_prompt_data = [''] * len(data['sampled'])
    # evaluate criterion
    ...
    for idx in tqdm.tqdm(range(n_samples), desc=f"Computing {name} criterion"):
        ...
        # sampled text
        sample_prompt_text = sampled_prompt_data[idx]
        prompt_tokenized = scoring_tokenizer(sample_prompt_text, return_tensors="pt", padding=True, return_token_type_ids=False).to(args.device)
        prompt_len = prompt_tokenized.input_ids.shape[1]
        tokenized = scoring_tokenizer(sample_prompt_text + sampled_text, return_tensors="pt", padding=True, return_token_type_ids=False).to(args.device)
        labels = tokenized.input_ids[:, prompt_len+1:]
        with torch.no_grad():
            logits_score = scoring_model(**tokenized).logits[:, prompt_len:-1]
            if args.sampling_model_name == args.scoring_model_name:
                logits_ref = logits_score
            else:
                tokenized = sampling_tokenizer(sample_prompt_text + sampled_text, return_tensors="pt", padding=True, return_token_type_ids=False).to(args.device)
                assert torch.all(tokenized.input_ids[:, prompt_len+1:] == labels), "Tokenizer is mismatch."
                logits_ref = sampling_model(**tokenized).logits[:, prompt_len:-1]
        ...
        if include_revised_text:
            ...
            with torch.no_grad():
                ...
                if args.sampling_model_name == args.scoring_model_name:
                    ...
                    assert torch.all(tokenized.input_ids[:, 1:] == labels), "Tokenizer is mismatch."
                    logits_ref = sampling_model(**tokenized).logits[:, :-1]
                revised_crit = criterion_fn(logits_ref, logits_score, labels)
    ...
    if not include_revised_text:
        ...
        results = { 'name': f'{name}_threshold',
                    ...
                    'loss': 1 - pr_auc, 
                    'compute_info': {'pre_time': 0.0, 'eval_time': eval_time, 
                                     'pre_memory': 0.0, 'eval_memory': eval_memory,}
                  }
    else:
        predictions = {'real': [x["original_crit"] for x in results],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_file', type=str, default="./exp_prompt/results/xsum_gemma-9b-instruct")  # output file prefix
    parser.add_argument('--dataset', type=str, default="xsum")
    parser.add_argument('--prompt_file', type=str, default=None)
    parser.add_argument('--dataset_file', type=str, default="./exp_prompt/data/xsum_gemma-9b-instruct")
    parser.add_argument('--sampling_model_name', type=str, default="gemma-9b-instruct")
